...
const Task: FC<TTask> = (props: TTask) => {
	const [active, setLocalActive] = useState(props.active)
	const [deleteAnim, setDeleteAnim] = useState(false)
	const { removeTask, setActive } = useActions()

	useEffect(() => {
		setActive({ id: props.id, active: active })
	}, [active])

	return (
		<div className={deleteAnim ? styles.task + ' ' + styles.anim : styles.task}>
			<label>
				<input
					type="radio"
					readOnly
					checked={!active}
					onClick={() => setLocalActive(!active)}
			...
			<p
				className={
					props.active ? styles.text : styles.text + ' ' + styles.checked
				}
			>
				{props.text}
			</p>
